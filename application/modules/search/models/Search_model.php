      ...
      public function get_search_results($search_array)
      {
          ...
          if (!empty($search_array))
          {
              ...
              if ($search_array['CVV2ResponseCode'] != NULL) {
                  $this->db->where('CVV2ResponseCode', $search_array['CVV2ResponseCode']);
              }

              if ($search_array['SerialNumber'] != NULL) {
                  $this->db->where('SerialNumber', $search_array['SerialNumber']);
				  $this->db->or_where('TransactionFileName', $search_array['SerialNumber']);
              }

			  if ($search_array['TransactionStatusId'] != NULL) {
                  $this->db->where('TransactionStatusId', $search_array['TransactionStatusId']);
              }
...
//               $sql = $this->db->get_compiled_select();
    ...
    public function get_classifiedad_totals()
    {
        $this -> db -> select('COUNT(*) AS num_ca, SUM(TransactionAmount) AS amount_ca');

        $this -> db -> from('cadatransactions');

        $this -> db -> where('PaymentSource', 'CA');

        return $this -> db -> get() -> row();
    }

    public function get_displayad_totals()
    {
        $this -> db -> select('COUNT(*) AS num_da, SUM(TransactionAmount) AS amount_da');

        $this -> db -> from('cadatransactions');

        $this -> db -> where('PaymentSource', 'DA');
      ...
      // This code was the old way of pulling all the tables for a transaction search.  Instead a view has been created in place of this query.
//    /**
//     * Pass in the search array and query the database
//     * Return the info from the database query
//     */
...
//
////              $this->db->join('classifiedad_submissions', 'payment_response.TransactionTypeId = classifiedad_submissions.id AND payment_response.PaymentSource = "CA"', 'left');
//
//            $this->db->join('displayad_submissions', 'payment_response.TransactionTypeId = displayad_submissions.id AND payment_response.PaymentSource = "DA"', 'left');
//
